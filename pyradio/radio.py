...
class PyRadio(object):
    ...
    def ss(self, msg):
        ...
        logger.error('DE NORMAL_MODE: {0}, {1}, {2}'.format(
            self.selections[0][0],
            self.selections[0][1],
            self.selections[0][2]))
        logger.error('DE PLAYLIST_MODE: {0}, {1}, {2}'.format(
            self.selections[1][0],
            self.selections[1][1],
            self.selections[1][2]))
        logger.error('DE REGISTER_MODE: {0}, {1}, {2}'.format(
            self.selections[2][0],
            self.selections[2][1],
            self.selections[2][2]))
    ...
    def run(self):
        if self.ws.operation_mode == self.ws.NO_PLAYER_ERROR_MODE:
            ...
            if CAN_CHECK_FOR_UPDATES and app_state == '':
                if self._cnf.locked:
                    if logger.isEnabledFor(logging.INFO):
    ...
    def _show_main_help(self):
        ...
        self._show_help(txt, mode_to_set=self.ws.MAIN_HELP_MODE, reset_metrics=False)
    ...
    def _show_main_help_page_2(self):
        ...
        self._show_help(txt, mode_to_set=self.ws.MAIN_HELP_MODE_PAGE_2, reset_metrics=False)
    ...
    def _show_playlist_help(self):
        txt = """Up|,|j|,|PgUp|,
                 ...
                 t| / |T            |Load |t|heme / |T|oggle transparency."""
        if self._cnf.open_register_list:
            self._show_help(txt, mode_to_set=self.ws.PLAYLIST_HELP_MODE, caption=' Registers List Help ')
        else:
    ...
    def _show_yank_help(self):
        ...
        txt = """ENTER        |Copy station to unnamed register.
                 ...
                 |Any other key exits current mode.
                 """
        self._show_help(txt, mode_to_set=self.ws.YANK_HELP_MODE, caption=' Copy Mode Help')
    ...
    def _align_stations_and_refresh(self, cur_mode, a_startPos=-1, a_selection=-1):
        ...
        #self.ss('_align_stations_and_refresh')
        self.refreshBody()
    ...
    def _clear_register_file(self):
        ...
        if self.ws.operation_mode == self.ws.NORMAL_MODE:
            ...
            file_to_remove = path.join(self._cnf.registers_dir, self.stations[self.selection][0] + '.csv')
        try:
    ...
    def _show_notification_with_delay(self, txt, mode_to_set, callback_function,
            delay=.75, reset_metrics=True):
        self._show_help(txt, mode_to_set, caption='',
                prompt='', is_message=True, reset_metrics=reset_metrics)
    ...
    def keypress(self, char):
        ...
        elif self.ws.operation_mode == self.ws.NEW_PLAYLIST_MODE:
            # Create new playlist
            if char in self._chars_to_bypass_on_editor and \
                    self._new_playlist_dialog.focus > 0:
                self.volume_functions[chr(char)]()
                return
            return

        elif self.ws.operation_mode == self.ws.RENAME_PLAYLIST_MODE:
            # Rename playlist
            if char in self._chars_to_bypass_on_editor and \
